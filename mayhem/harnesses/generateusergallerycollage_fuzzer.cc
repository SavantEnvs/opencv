...
// Additive: the putText() call is wrapped in try/catch. In 5.x the FONT_HERSHEY_* entry point
// (modules/imgproc/src/drawing_text.cpp) maps each Hershey face onto a built-in TrueType font and
// renders through the bundled HarfBuzz shaper + hb-raster rasterizer; that path reports invalid
// arguments (empty canvas, zero size) via cv::Exception, which the 4.x vector-glyph renderer never
// threw. Swallowing it keeps every other input fuzzing; sanitizer findings are unaffected.
//
// Surface: fuzzer-controlled UTF-8 text (arbitrary bytes), font face, scale, weight, line type,
// origin and canvas size (up to 6000x6000 CV_8UC3) -> cv::putText -> HarfBuzz shaping (OpenType
// tables of the bundled Rubik fonts) -> glyph rasterization -> alpha blending into the canvas.
