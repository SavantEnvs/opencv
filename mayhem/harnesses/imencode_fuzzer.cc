...
// Known finding (mayhem/imencode_fuzzer/known-findings/sunras-encoder-odd-row-overread/): the Sun Raster
// encoder over-reads one byte of every odd-width row (`img.cols * img.channels()` odd). Mayhem's
// accumulated corpus now carries inputs that hit it deterministically, and one crash during the
// `-runs=5` sanity probe aborts the whole run before fuzzing starts (savantenvs/opencv imencode run #6).
// Skip exactly that class so the target stays fuzzable; every other codec, and every even-width `.ras`
// encode, is unchanged. Build with -DIMENCODE_FUZZER_KEEP_KNOWN_FINDINGS to reproduce the finding.
bool SkipKnownFinding(const char* ext, const cv::Mat& img) {
#ifdef IMENCODE_FUZZER_KEEP_KNOWN_FINDINGS
  (void)ext; (void)img;
  return false;
#else
  return std::strcmp(ext, ".ras") == 0 &&
         ((static_cast<long long>(img.cols) * img.channels()) & 1LL) != 0;
#endif
}

void Encode(const char* ext, const cv::Mat& img) {
  if (SkipKnownFinding(ext, img)) return;
  try {
